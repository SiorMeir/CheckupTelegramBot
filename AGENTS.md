...
A Python Telegram bot that parses markdown **daily check-ins** and **weekly reviews**, plus scheduled reminders for a configured chat. Parsed replies are shown in chat only — **persistence, statistics, and review aggregation are not implemented yet** (see [Not implemented](#not-implemented-vs-architecturemd)).
...
1. **Telegram bot** (`app.py`) — [python-telegram-bot](https://github.com/python-telegram-bot/python-telegram-bot) v22 polling app with:
   ...
   - **Text messages** — if awaiting daily or weekly, runs `CheckupParser.parse`, validates `DailyCheckIn` vs `WeeklyReview`, and replies with `format_parsed_daily` or `format_parsed_weekly`; otherwise replies that nothing is armed
...
|------|----------------|--------|
| Storage | PVC + `journal/daily/`, `journal/weekly/` `.md` files | Not started |
...
| LLM / patterns | Feed stored journal data for tracking | Blocked on storage |
...
Implement persistence first; it unblocks statistics, review, and LLM export.
